...
for (dir=0; dir<=11; dir++)
	...
	if m
		{
		locate (automaplib.CENTER_MAP_HORZ + dir - 4), automaplib.CENTER_MAP_VERT

		if from is visited and
		automaplib.CENTER_MAP_HORZ >= 1 and automaplib.CENTER_MAP_HORZ <=		display.linelength and
		automaplib.CENTER_MAP_VERT >= 1 and automaplib.CENTER_MAP_VERT <= (display.windowlines )
			{
			if from.m
				print map_chars.m;
			else
  ...
  {
    locate (automaplib.CENTER_MAP_HORZ - 1), (automaplib.CENTER_MAP_VERT - 1)
    if (automaplib.CENTER_MAP_HORZ - 1) >= 1 and (automaplib.CENTER_MAP_HORZ - 1) <= display.linelength and
       (automaplib.CENTER_MAP_VERT - 1) >= 1 and (automaplib.CENTER_MAP_VERT - 1) <= (display.windowlines) ! - 2
    { if from.w_to
      { g = from.w_to
  ...
  {
    locate (automaplib.CENTER_MAP_HORZ + 2), (automaplib.CENTER_MAP_VERT - 1)
    if (automaplib.CENTER_MAP_HORZ + 2) >= 1 and (automaplib.CENTER_MAP_HORZ + 2) <= display.linelength and
       (automaplib.CENTER_MAP_VERT - 1) >= 1 and (automaplib.CENTER_MAP_VERT - 1) <= (display.windowlines) ! - 2
    { print "|";
    }
    locate 1,1
  ...
  { locate (automaplib.CENTER_MAP_HORZ + 4), (automaplib.CENTER_MAP_VERT - 1)
    if (automaplib.CENTER_MAP_HORZ + 4) >= 1 and (automaplib.CENTER_MAP_HORZ + 4) <= display.linelength and
       (automaplib.CENTER_MAP_VERT - 1) >= 1 and (automaplib.CENTER_MAP_VERT - 1) <= (display.windowlines) ! - 2
    { if from.e_to
      { g = from.e_to
  ...
  { locate (automaplib.CENTER_MAP_HORZ - 1), automaplib.CENTER_MAP_VERT
    if (automaplib.CENTER_MAP_HORZ - 1) >= 1 and (automaplib.CENTER_MAP_HORZ - 1) <= display.linelength and
       (automaplib.CENTER_MAP_VERT) >= 1 and (automaplib.CENTER_MAP_VERT) <= (display.windowlines) ! - 2
    { print "-";
    }
    locate 1,1
  }
  ...
  if from.sw_to and from is visited
	...
	locate (automaplib.CENTER_MAP_HORZ - 1), (automaplib.CENTER_MAP_VERT + 1)
	if (automaplib.CENTER_MAP_HORZ - 1) >= 1 and (automaplib.CENTER_MAP_HORZ - 1) <= display.linelength and
	(automaplib.CENTER_MAP_VERT + 1) >= 1 and (automaplib.CENTER_MAP_VERT + 1) <= (display.windowlines)  ! changed south to -1 from -2
		{
		if from.w_to
			{ g = from.w_to
			 if g.se_to and g is visited and g ~= from
  ...
  m = 0
    select dir
      ...
      { if loc.sw_to
        { locate (startpos - 1), (startrow + 1)
          if (startpos - 1) >= 1 and (startpos - 1) <= display.linelength and
             (startrow + 1) >= 1 and (startrow + 1) <= (display.windowlines)
          { print "/";
          }
        }
      }
...
routine MapSet ! draws the map in a window in regular interpreters
{
	...
	if not system(61) ! not glk in this case
	{
		if automaplib.AUTOMAP_ON       ! The call to Map() using the switch
		{
			...
			window 1, (display.statusline_height + 1), display.screenwidth, (display.statusline_height + 9)
			{
				cls
			}
			color TEXTCOLOR, BGCOLOR
			window (display.screenwidth - 20) , (display.statusline_height + 2), display.screenwidth , (display.statusline_height + 9)
			{
			   Map()
			}
...
window display.statusline_height
	...
	if glktest and (automaplib.MAP_ON or automaplib.AUTOMAP_ON or automaplib.FULL_ON )
		{
		""
...
routine AutomapMessage(r, num, a, b)
{
	...
	select r
		...
		case &FullMap
			...
			select num
				case 1: print "done";
				case 2: print to (display.linelength/2 - 11); "PRESS A KEY TO CONTINUE";
			}
}
